...
class Ajax extends Backend
{
    ...
    public function upload($filetype = "")
    {
        ...
        if ($splInfo)
        {
            ...
            if (in_array($suffix, ['gif', 'jpg', 'jpeg', 'bmp', 'png', 'swf', 'mp4', 'rmvb', 'avi', 'flv']))
            {
            ...
            $params = array(
                ...
                'url'         => $website . $uploadDir . $splInfo->getSaveName(),
                'uploadtime'  => time(),
                'storage'     => 'local',
                'sha1'        => $sha1,
            );
            ...
            if($filetype == 'video'){
                $url = $this->base64_en($uploadDir . $splInfo->getSaveName());
                echo "<script>parent.showimg('{$url}');</script>";
                exit;
            }
     ...
     * 读取考试分类数据,联动列表
     */
    public function category_papers()
    {
        $pid = $this->request->get('pid');
        $where = [
            'status' => 'normal',
