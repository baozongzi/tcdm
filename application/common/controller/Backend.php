...
class Backend extends Controller
{
     ...
     * Multi方法可批量修改的字段
     */
    protected $multiFields = 'status,papers';
    ...
    public function artist_search(){
        ...
        $normal_name = input('normal_name');
        $sex = input('sex');
        $where = "role = 1";
        if($normal_name !== ""){
            $where .= " AND normal_name = '".$normal_name."'";
        }
        if($sex !== ""){
            $where .= " AND sex = '".$sex."'";
        }
        $data = $this->user->where($where)->field('id,normal_name,sex,age,head')->select();
        return $data;
    }
    ...
    public function artist_handles($lastid,$params,$users,$table){
        ...
        $artist = array();
        if(!empty($userid)){
            foreach ($userid as $uid => $user) {
                foreach ($cosplay as $c => $play) {
                    $artist[$uid]['userid'] = $userid[$uid];
                    $artist[$uid]['cosplay'] = $cosplay[$uid];
                }
            }
        }
        ...
        try {
            if($lastid){
                $this->model->save($params,['id' => $lastid]);
            }else{
                $this->model->save($params);
                $lastid = $this->model->getLastInsID();
            }
            Db::execute("delete FROM fa_representative where vid = ".$lastid." AND model = '".$table."'");
            foreach ($artist as $a => $art) {
                // 插入记录
                Db::execute("insert into fa_representative (uid, vid ,model) values (".$art['userid'].", ".$lastid.",'".$table."')");       
            }
            $result = '1';
            Db::commit(); //提交事务
    ...
    public function tree(&$data,$parentid = 0,$level = 0,$sign=' └ ') {
